const { facPools, INITIAL_FAC_FOR_POOLS } = require('./pools');
...
module.exports = async (deployer, network, accounts) => {
  ...
  const initialCashAmount = unit.muln(INITIAL_FAC_FOR_POOLS).toString();

  const cash = await Cash.deployed();
  const pools = facPools.map(({contractName}) => artifacts.require(contractName));
  ...
  console.log(`Deposited ${INITIAL_FAC_FOR_POOLS} FAC to InitialCashDistributor.`);
